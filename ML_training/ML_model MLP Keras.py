...
def train_keras_MLPmodel(df, dataset_name=('default')):
    ...
    MLP_model.add(Dense(16, input_shape=(len(featureslist),), activation='relu', name='hidden_layer_1'))
    MLP_model.add(Dense(8, activation='relu', name='hidden_layer_2'))
    ...
    MLP_model.fit(x_train_scaled, y_train, epochs=30, batch_size=32, verbose=0)
    MLP_predictions=(MLP_model.predict(x_test_scaled)>0.5).astype("int32")
    print(f"MLP Accuracy: {accuracy_score(y_test, MLP_predictions): .4f}")
    np.save(f'Trained Data/keras_MLP_{dataset_name}_predictions.npy', MLP_predictions)
    np.save(f'Trained Data/keras_MLP_{dataset_name}_y_test.npy', y_test[:100])
    np.save(f'Trained Data/keras_MLP_{dataset_name}_x_test.npy', x_test_scaled[:100])
    print((f'Saved model and testing data in {model_location}'))
    MLP_model.save(f'Trained Data/keras_MLP_{dataset_name}.h5')
...
if __name__ == "__main__":
    ...
    train_keras_MLPmodel(df, dataset_name= "April_2023") #update to new date each time
